...
from .class_utils import keys_vocab, iob_labels_vocab
...
def iob_index_to_str(tags: List[List[int]]):
    ...
    for doc in tags:
        decoded_tags = []
        for tag in doc:
            s = iob_labels_vocab.get_itos()[tag]
            if s == '<unk>' or s == '<pad>':
                s = 'O'
...
def text_index_to_str(texts: torch.Tensor, mask: torch.Tensor):
    ...
    for i in range(B):
        ...
        for text_index in union_texts[i]:
            text_str = keys_vocab.get_itos()[text_index]
...
def texts_to_union_texts(texts, mask):
    ...
    union_texts = torch.full_like(texts, keys_vocab['<pad>'], device=texts.device)
...
def iob_tags_to_union_iob_tags(iob_tags, mask):
    ...
    # union tags as a whole sequence, (B, N*T)
    union_iob_tags = torch.full_like(iob_tags, iob_labels_vocab['<pad>'], device=iob_tags.device)
